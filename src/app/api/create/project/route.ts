...
const rolePrompt: string = `You are an expert at identifying technical roles from project descriptions.  
...
export async function POST(request: Request) {
    try {
        ...
        if (decoded && typeof decoded === "object" && "plan" in decoded) {
            ...
            let preBody = "";
            if (body.name) {
                preBody += `Project Name: ${body.name}, `;
            }
            if (body.tech) {
                preBody += `Technologies Used: ${body.tech}, `;
            }
            if (body.role) {
            ...
            if (plan == "pro") {
                ...
                if (role) {
                        ...
                        "messages": [
                            {
                                "role": "system",
                                "content": projectPromptWithoutRole
                            },
                            {
                                "role": "user",
                ...
                const chatCompletion = await groq.chat.completions.create({
                    "messages": [
                        ...
                        {
                            "role": "user",
                            "content": "Name: " + body.name + " " + body.description
                        },
                    ],
                    "model": "openai/gpt-oss-20b",
                    "temperature": 1,
        ...
        return new Response(JSON.stringify({ error: "Invalid or expired token", env:process.env, eee: error}));
    }
}
