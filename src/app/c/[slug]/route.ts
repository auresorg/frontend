...
import type { Project, Education, Certification, Experience, Award } from "@/lib/types";
...
async function fetchCusresData(slug: string) {
  ...
  return { payload, username: base.username, cusresId: base.cusres_id };
}
...
export async function GET(
  ...
  try {
    ...
    if (
      cache[0]?.compiled_at &&
      new Date(cache[0].compiled_at) >= new Date(cache[0].data_updated_at)
    ) {
    ...
    if (!gen.ok) {
      ...
      return NextResponse.json({ error: "resgen failed", body }, { status: 500 });
    }

    const pdf = Buffer.from(await gen.arrayBuffer());
    const filename = `${data.username}-${slug}.pdf`;
  ...
  try {
    ...
    return new NextResponse(await gen.text(), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${data.username}-${slug}.tex"`,
      },
    });
  } catch {
