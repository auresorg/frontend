...
export default function EditExperienceDialog({ experience, onClose, onSave }: EditExperienceDialogProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const updatedExperience = {
            ...experience,
            ...formData,
            role: formData.role as Experience['role']
        };

        try {
            const response = await putWithToken(`/experiences/${experience.id}`, updatedExperience);
            if (response && response.status === 200) {
                toast({
                    title: 'Experience updated',
                    description: `"${updatedExperience.title}" has been updated successfully.`,
                    variant: 'success',
                });
    ...
    const handleAIRephrase = async () => {
        ...
        try {
            const response = await postWithTokenNextEndpoint("/create", { ...formData, type: "experience" });
            ...
            <DrawerContent className="overflow-hidden sm:max-w-lg dark:bg-gray-925">
                ...
                <DrawerBody className="overflow-y-auto">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        {/* Company */}
                        <div>
                            <Label htmlFor="edit-company" className="font-medium">
                        ...
                        {/* Start Date */}
                        <div>
                            <Label htmlFor="edit-startDate" className="font-medium">
                        ...
                        {/* End Date */}
                        <div>
                            <Label htmlFor="edit-endDate" className="font-medium">
                                End Date
                            </Label>
                            <Input
                                type="date"
                            ...
                            <style jsx>
                                {`
                                    @keyframes energySweep {
                                        0% { transform: translateX(-120%) skewX(-8deg) scaleX(1.2); opacity: 0.2; filter: blur(14px); }
                                        20% { opacity: 0.8; }
                                        50% { transform: translateX(0%) skewX(0deg) scaleX(1); opacity: 1; filter: blur(6px); }
                                        80% { opacity: 0.7; }
                                        100% { transform: translateX(120%) skewX(8deg) scaleX(1.2); opacity: 0; filter: blur(14px); }
                                    }
                                    .animate-energy-sweep { position: absolute; inset: 0; background: conic-gradient(from 120deg at 30% 50%, rgba(0,122,255,.25), rgba(147,51,234,.25), rgba(236,72,153,.25), rgba(0,122,255,.25)); background-size: 200% 200%; animation: energySweep 3.2s cubic-bezier(0.45,0.1,0.15,1) forwards; filter: saturate(160%) brightness(1.15); mix-blend-mode: screen; border-radius: 6px; mask-image: radial-gradient(circle at 50% 50%, black 60%, transparent 100%); }
                                    @keyframes glowBorder { 0% { box-shadow: 0 0 8px rgba(0,122,255,.25), 0 0 16px rgba(147,51,234,.1), inset 0 0 8px rgba(236,72,153,.08); } 50% { box-shadow: 0 0 24px rgba(0,122,255,.7), 0 0 36px rgba(147,51,234,.4), inset 0 0 14px rgba(236,72,153,.2); } 100% { box-shadow: 0 0 8px rgba(0,122,255,.25), 0 0 16px rgba(147,51,234,.1), inset 0 0 8px rgba(236,72,153,.08); } }
                                    .animate-glow-border { animation: glowBorder 2.8s ease-in-out infinite; }
                                    @keyframes unravelReveal { 0% { -webkit-mask-position: 100% 0%; mask-position: 100% 0%; } 100% { -webkit-mask-position: 0% 0%; mask-position: 0% 0%; } }
                                    .animate-text-reveal { -webkit-mask-image: repeating-linear-gradient(90deg, rgba(0,0,0,1) 0px, rgba(0,0,0,1) 10px, rgba(0,0,0,0) 14px, rgba(0,0,0,0) 24px); mask-image: repeating-linear-gradient(90deg, rgba(0,0,0,1) 0px, rgba(0,0,0,1) 10px, rgba(0,0,0,0) 14px, rgba(0,0,0,0) 24px); -webkit-mask-size: 200% 100%; mask-size: 200% 100%; animation: unravelReveal 1.6s ease-out forwards; }
                                    textarea[disabled] { cursor: progress; color: transparent !important; text-shadow: none; }
                                `}
                            </style>
                        </div>
